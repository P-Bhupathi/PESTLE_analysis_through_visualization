...
function updateSelectedDateRange_bubble(year,id,canvas_id) {
    //console.log(year)
    $('#'+id).val(year)
    time_vs_intensity_and_likelihood(year,canvas_id)
}
window.updateSelectedDateRange_bubble = updateSelectedDateRange_bubble;


// Event for change date of jquery input for INTENSITY_vs_year graph 
function updateSelectedDateYear_bubble(this_id,range_id,canvas_id) {
    var enteredYear = $('#'+this_id).val();
        ...
        time_vs_intensity_and_likelihood($('#'+this_id).val(),canvas_id);
}
window.updateSelectedDateYear_bubble = updateSelectedDateYear_bubble;