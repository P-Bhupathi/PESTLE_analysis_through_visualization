function time_vs_intensity_of_pestle(year=2200,canvas_id) {
    //fetch(`/time_vs_intensity/${year}/`)
        //.then(response => response.json())
            //.then(time_vs_intensity => {

            var data = global_data;
            var lst = ['sector','topic','region','country','source']
            ...
            var mappedAndFilteredData = data
                                        .filter(item => item.end_year == year && item.end_year && item.intensity && item.pestle) // Filter items where intensity is greater than 100
                                        .map(item => ({
            ...
            var groupedData = mappedAndFilteredData.reduce((result, item) => {
                var key = item.pestle;
                if (!result[key]) {
                    result[key] = { pestle: item.pestle, intensity: 0};
                }
            ...
            new Chart(ctx, {
                type: 'pie',
                data: {
                        labels: dataArray.map(item=>item.pestle),
                        datasets: [{
